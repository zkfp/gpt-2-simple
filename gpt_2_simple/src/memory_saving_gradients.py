...
def gradients(ys, xs, grad_ys=None, checkpoints="collection", **kwargs):
    ...
    fwd_ops = ge.get_forward_walk_ops(
        [x.op for x in xs], inclusive=True, within_ops=bwd_ops
    )
    ...
    debug_print(
        "ys: %s, checkpoints: %s, intersect: %s",
        ys,
        checkpoints,
    ...
    debug_print(
        "Found %s ops to copy within fwd_ops %s, seed %s, stop_at %s",
        len(ops_to_copy),
        fwd_ops,
        [r.op for r in ys],
        checkpoints,
    ...
    debug_print(
        "Rewired %s in place of %s restricted to %s",
        checkpoints_disconnected.values(),
        checkpoints_disconnected.keys(),
        copied_ops,
    )
    ...
    d_checkpoints = {
        r: dr
        for r, dr in zip(
            checkpoints_disconnected.keys(), dv[: len(checkpoints_disconnected)]
        )
    }
    ...
    for ts in checkpoints_sorted_lists[::-1]:
        ...
        debug_print(
            "Found %s ops to copy within %s, seed %s, stop_at %s",
            len(ops_to_copy),
            fwd_ops,
            [r.op for r in ts],
            checkpoints_other,
        ...
        debug_print("Copied %s to %s", ops_to_copy, copied_ops)
        ge.reroute_ts(
            checkpoints_disconnected_other, checkpoints_other, can_modify=copied_ops
        )
        debug_print(
            "Rewired %s in place of %s restricted to %s",
            checkpoints_disconnected_other,
