...
try:
    # Only works in Google Colaboratory.
...
from gpt_2_simple.src.ada_optimizer import (
    AdafactorOptimizer,
...
def start_tf_sess(threads=-1, server=None):
    """Returns a tf.Session with config."""
...
def reset_session(sess, threads=-1, server=None):
    """Resets the TensorFlow session, to clear memory or load another model."""
...
def generate(
    ...
    output = sample.sample_sequence(
        hparams=hparams,
        length=min(length, 1023 - len(context_tokens)),
